...
(function($) {
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			// Reset some queue info nwc
			this.queueData.errorMsg       = 'Warning！';
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					//var replaceQueueItem = confirm('文件 "' + file.name + '" 在队列中已存在.是否需要替换?');
					var replaceQueueItem = confirm(' File  "' + file.name + '" exists in the queue , whether to replace to continue?');
					if (!replaceQueueItem) {
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							 this.queueData.errorMsg += '\nThe number of files selected exceeds the remaining upload limit (' + settings.queueSizeLimit + ').';
							//nwcthis.queueData.errorMsg += '\n最大上传限制个数为 (' + settings.queueSizeLimit + ')个。';
						} else {
					...
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						...
						//this.queueData.errorMsg += '\n文件最大支持'+ settings.fileSizeLimit + ').';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\nThe file "' + file.name + '"  name is empty.';
						//this.queueData.errorMsg += '\n "' + file.name + '" 不能为空.';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
